#!/usr/bin/env python3
"""
Modified GVHMR demo that adds skeleton visualization

This is a wrapper around the original demo.py that adds:
1. Skeleton overlay on input video (skeleton_incam.mp4)
2. Skeleton on black background (skeleton_only.mp4)
3. Joint positions in JSON format (joints.json)
...
# Run original demo first - import after path is set
import cv2
import torch
...
# Add web_app to path for skeleton renderer
sys.path.insert(0, '/app/web_app')
from render_skeleton import render_skeleton_incam, render_skeleton_only, save_joints_json
